...
const source = fs.readFileSync(
  path.join(process.cwd(), 'js/kanban.js'),
  'utf8'
);
...
jscs.registerMethods({
  findFunction: function (name) {
    const element = this.find(jscs.Identifier, { name: name }).filter(
      (path) => {
        if (path.parent.value.type === 'VariableDeclarator') {
          if (
            path.parent.value.init.type === 'FunctionExpression' ||
  ...
  findVariable: function (name) {
    return this.find(jscs.VariableDeclarator).filter(
      (path) => path.value.id.name === name
    );
  },
  ...
  findAssignment: function (name) {
    return this.find(jscs.AssignmentExpression).filter(
      (path) =>
        path.value.left.type === 'Identifier' && path.value.left.name === name
    );
  },
  findUpdate: function (name, operator) {
    return this.find(jscs.UpdateExpression).filter(
      (path) =>
        path.value.argument.name === name && path.value.operator === operator
    );
  },
  ...
  findLiteral: function (name) {
    const element = this.find(jscs.Literal).filter(
      (path) => path.value.value === name
    );
    return element.length ? jscs(element.get().parent) : [];
  },
});

const matchObj = (obj, match_obj) =>
  obj.length ? jscs.match(obj.get().value, dot.object(match_obj)) : false;
const paramLength = (obj) =>
  obj.length ? (obj.get().value.arguments.length ? true : false) : false;
...
const matchParam = (obj, other, level = false) => {
  if (obj.length && other.length) {
    ...
    if (
      checkNested(obj_value, 'arguments') &&
      ...
      if (level) {
        if (checkNested(other_value.arguments[1], 'object', 'object', 'name')) {
          return mEqual(
            obj_value.arguments[1].params[0].name,
            other_value.callee.object.object.name,
            other_value.arguments[1].object.object.name
          );
        } else {
          return false;
        }
...
const findEventParam = (obj) => {
  if (obj.length) {
    const obj_value = obj.get().value;
    return checkNested(obj_value, 'arguments') &&
      obj_value.arguments.length >= 2 &&
      obj_value.arguments[1].params.length
      ? obj_value.arguments[1].params[0].name
      : false;
