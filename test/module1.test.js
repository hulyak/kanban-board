describe('Module 01 - Kanban Board Events - kanban.js', () => {
  ...
  it('Should create an `item` `div` element. @create-item', () => {
    ...
    assert(
      matchObj(create_item_assignment, create_item_match),
      'Are you creating a variable called `item` and assigning it a call to the `createElement()` function'
    );
  });
  ...
  it('Should set item attributes. @set-attributes', () => {
    ...
    assert(
      matchObj(add_class, add_class_match),
    ...
    assert(
      matchObj(item_id, item_id_match),
      "Are you giving the `item` the `id` of `'item-' + order`?"
    );

    const item_draggable = create_item.findPropertyAssignment(
      'item',
      'draggable'
    );
    ...
    assert(
      matchObj(item_draggable, item_draggable_match),
      'Are you making the `item` draggable?'
    );
    item_draggable;
  });
  ...
  it('Should register a listener for the `dragstart` event. @dragstart-event-listener', () => {
    ...
    assert(
      matchObj(drag_start, drag_start_match),
      'Are you adding an event listener to `item` that listens for the `dragstart` event?'
    );

    assert(
      matchObj(drag_start, { 'arguments.1.type': 'ArrowFunctionExpression' }) ||
        matchObj(drag_start, { 'arguments.1.type': 'FunctionExpression' }),
      'Do you have a `dragend` handler function?'
    );

    assert(
      paramLength(drag_start),
      'Does your handler function have a single parameter?'
    );
    ...
    assert(
      matchParam(drag_start, set_data, true),
      'Are you using the same `event` parameter name?'
    );
    assert(
      matchObj(set_data, set_data_match),
      'In the `dragstart` event handler are you setting the `text` of the `dataTransfer` to `event.target.id`?'
    );
  });
  ...
  it('Should register a listener for the `dragend` event. @dragend-event-listener', () => {
    ...
    assert(
      matchObj(drag_end, drag_end_match),
      'Are you adding an event listener to `item` that listens for the `dragend` event?'
    );

    assert(
      matchObj(drag_end, { 'arguments.1.type': 'ArrowFunctionExpression' }) ||
        matchObj(drag_end, { 'arguments.1.type': 'FunctionExpression' }),
      'Do you have a `dragend` handler function?'
    );

    assert(
      paramLength(drag_end),
      'Does your handler function have a single parameter?'
    );
    ...
    assert(
      matchParam(drag_end, clear_data),
      'Are you using the same `event` parameter name?'
    );
    assert(
      matchObj(clear_data, clear_data_match),
      'In the `dragend` event handler are you setting clear all `dataTransfer` data?'
    );
  });
  ...
  it('Should create an `input` element. @create-input', () => {
    ...
    assert(
      matchObj(create_input_assignment, create_input_match),
    ...
    assert(
      matchObj(append_child, append_child_match) ||
        matchObj(append, append_match),
      'Are you appending `input` to `item`?'
    );
  });
  ...
  it('Should create a save button. @create-save-btn', () => {
    ...
    assert(
      matchObj(create_save_btn, create_save_btn_match),
      'Are you creating a variable called `save_btn` and assigning it a call to the `createElement()` function'
    );

    const save_btn_html = create_item.findPropertyAssignment(
      'save_btn',
      'innerHTML'
    );
    const save_btn_text = create_item.findPropertyAssignment(
      'save_btn',
      'textContent'
    );
    ...
    assert(
      matchObj(save_btn_html, save_btn_match) ||
        matchObj(save_btn_text, save_btn_match),
      'Are you setting the HTML of the button to `Save`?'
    );
  });
  ...
  it('Should register a `save_btn` click event listener. @click-event-listener', () => {
    ...
    assert(
      matchObj(save_btn_click, save_btn_click_match),
      'Are you adding a `click` event listener to the `save_btn` button?'
    );
    const save_btn_handler_arrow = {
      'arguments.1.type': 'ArrowFunctionExpression',
    };
    const save_btn_handler_function = {
      'arguments.1.type': 'FunctionExpression',
    };
    assert(
      matchObj(save_btn_click, save_btn_handler_arrow) ||
        matchObj(save_btn_click, save_btn_handler_function),
      'Do you have a `click` handler function?'
    );
  });
  ...
  it('Should validate user input with an `if` statement. @valid-input-if', () => {
    ...
    const error_text = create_item.findPropertyAssignment(
      'error',
      'textContent'
    );
    const error_match = {
    ...
    assert(
      matchObj(error_html, error_match) || matchObj(error_text, error_match),
      'Are setting the HTML of `error` to an empty string?'
    );

    assert(
      jscs(validate_if).length,
      'Have you created an `if` statement in the `save_btn` event handler?'
    );
    assert(
      (validate_if.test.operator === '!==' ||
        validate_if.test.operator === '!=') &&
        ((validate_if.test.right.value === '' &&
          validate_if.test.left.object.name === 'input' &&
          validate_if.test.left.property.name === 'value') ||
          (validate_if.test.left.value === '' &&
            validate_if.test.right.object.name === 'input' &&
            validate_if.test.right.property.name === 'value')),
  ...
  it('Should have an `if` statement body. @valid-input-if-body', () => {
    ...
    assert(
      matchObj(if_order_ap, if_order_ap_match) ||
        matchObj(if_order_ap, if_order_full_left_match) ||
        matchObj(if_order_ap, if_order_full_right_match) ||
        matchObj(if_order_pp, if_order_pp_match),
      'In the `if` statement, are adding `1` to `order`?'
    );
    ...
    assert(
      matchObj(if_item_html, if_item_match) ||
        matchObj(if_item_html, if_item_text),
      'Are you setting the HTML of `item` to the `input` value?'
    );
    ...
    assert(
      matchObj(if_adding, if_adding_match),
      'Are you setting the `adding` boolean to `false`?'
    );
  });
  ...
  it('Should have an `else` statement. @valid-input-else', () => {
    ...
    assert(
      matchObj(error_html, error_match) || matchObj(error_text, error_match),
      'In the `else` statement, are you setting the HTML of `error` to `message`?'
    );
  });
  ...
  it('Should append `save_btn` and return `item`. @append-save-btn-return', () => {
    ...
    assert(
      match(append_child, append_child_match) || match(append, append_match),
      'Below the `save_btn` event listener, are you appending `save_btn` to `item`?'
    );

    const return_item = create_item.find(jscs.ReturnStatement, {
    ...
    assert(
      matchObj(return_item, return_item_match),
      'Has `item` been returned from the `create_item` function?'
    );
  });
  ...
  it('Should register a listener for the `drop` event. @drop-event-listener', () => {
    ...
    assert(
      matchObj(drop, drop_match),
      'Are you adding an event listener to `element` that listens for the `drop` event?'
    );
    assert(
      matchObj(drop, { 'arguments.1.type': 'ArrowFunctionExpression' }) ||
        matchObj(drop, { 'arguments.1.type': 'FunctionExpression' }),
      'Do you have a `drop` handler function?'
    );
    assert(
      paramLength(drop),
      'Does your handler function have a single parameter?'
    ...
    assert(
      matchObj(prevent_default, prevent_default_match),
      'Are you calling `preventDefault` on `event` in the `drop` handler function?'
    );
  });
  ...
  it('Should get the `id` from `dataTransfer`. @drop-data-transfer-id', () => {
    ...
    assert(
      matchObj(id_get_data, id_get_data_match),
      "Are you creating a constant called `id` and setting it to a call to `event.dataTransfer.getData()` and passing in `'text'`?"
    );
  });
  ...
  it('Should append element. @drop-append-element', () => {
    ...
    assert(
      matchObj(drop_append_child, drop_append_child_match),
      'Are you appending the element with the `id` of `id` (use: `document.getElementById`) to the `event.target`?'
    );
  });
  ...
  it('Should register a listener for the `dragover` event. @drag-over-event-listener', () => {
    ...
    assert(
      matchObj(drag_over, drag_over_match),
    ...
    assert(
      matchObj(drag_over, drag_over_handler_arrow) ||
        matchObj(drag_over, drag_over_handler_function),
      'Do you have a `dragover` handler function?'
    );
    ...
    assert(
      matchObj(prevent_default, prevent_default_match),
      'Are you calling `preventDefault` on `event` in the `dragover` handler function?'
    );
  });
});
